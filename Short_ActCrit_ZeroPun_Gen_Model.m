...
for i=1:numSubj
    ...
    rewardTable(:,4:8) = genRewardTable; %collapse reward tables
...
%     rewardTable(rewardTable(:,:) > 1) = 5;
    ...
    for k=1:numRounds-1
        if (resultsMatrix(k, 2) == 4)
            if (resultsMatrix(k+1,1) == resultsMatrix(k,1))
                if (resultsMatrix(k, 3) < 0)
                    numStay(1) = numStay(1) + 1;
                    out_data(end+1,:) = [0,i+numSubj,1,0]; % [short, agentN, neg, stay]
                elseif (resultsMatrix(k, 3) > 0)
                    numStay(3) = numStay(3) + 1;
                else
                    numStay(2) = numStay(2) + 1;
                    out_data(end+1,:) = [0,i+numSubj,0,0]; % [short, agentN, zero, stay]
                end
            elseif (resultsMatrix(k, 3) < 0)
                numGo(1) = numGo(1) + 1;
                out_data(end+1,:) = [0,i+numSubj,1,1]; % [short, agentN, neg, go]
            elseif (resultsMatrix(k, 3) > 0)
                numGo(3) = numGo(3) + 1;
            else
                numGo(2) = numGo(2) + 1;
                out_data(end+1,:) = [0,i+numSubj,0,1]; % [short, agentN, zero, go]
            end
        end
    end
